...
import com.aptitude.learning.e2buddy.School.Admin.AdapterClass.StudentTestAdapter;
...
public class AdminViewStudentActivity extends AppCompatActivity{

    private int adminId,schoolId,classTestId,classId,maxMarks,passMarks;
    private String adminName, adminEmail, schoolCode, adminImage, schoolName,schoolImage,subjectName,testName,examDate;
    private AdminData user;
    private SchoolData schoolData;
    private TextView tvSchoolName,tvClassName,tvSectionName,tvTestName,tvMaxMarks,tvSubjectName,tvMsg;
    private CircleImageView imgSchoolLogo;
    private RecyclerView recyclerViewStudents;
    private List<StudentTestData> list;
    ...
    private StudentTestAdapter studentTestAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        studentTestAdapter = new StudentTestAdapter(AdminViewStudentActivity.this, list,passMarks,classTestId, dbHelper);

        getSchoolSection();
        tvSubjectName.setText(""+subjectName);
        tvTestName.setText(""+testName);
        tvMaxMarks.setText("Date : "+examDate);

        GetSectionName getSectionName = new GetSectionName(AdminViewStudentActivity.this, classTestId,tvSectionName);
        ...
        Glide.with(AdminViewStudentActivity.this)
    ...
    private void getStudents(final String sectionId){
        ...
        if (userDataList.size()>0) {
            ...
            for (int i = 0; i < userDataList.size(); i++) {
                ...
                getTestData(list, userData.getUserName(), userData.getSectionName(), userData.getUserId(), userData.getRollNo());
            }
        }
        else {
    ...
    private void getTestData(final List<StudentTestData> list, final String userNmae, final String sectionName, final int userId, String rollNo){

        MarkData markData = dbHelper.getStudentResult(classTestId,userId);

        list.add(new StudentTestData(userId, userNmae, sectionName, markData.getClassTestMark(), markData.getTeacherRemark(), rollNo));

        Collections.sort(list, new Comparator<StudentTestData>() {
            @Override
            public int compare(final StudentTestData object1, final StudentTestData object2) {

                return object2.getMarks() - object1.getMarks() ;
    ...
    public void getSchoolSection(){
        ...
        List<SectionData> sectionDataList = dbHelper.getSection(classTestId);
